 ...
 * Class definition for a rotation quaternion.
 *
 * @author: Eder A. Perez.
 */
...
public:
    ...
    void setRotation( T axisX, T axisY, T axisZ, T angle )
    {
        ...
        if( std::abs( axisX ) > Math< T >::EPSILON || std::abs( axisY ) > Math< T >::EPSILON ||
            std::abs( axisZ ) > Math< T >::EPSILON )
        {
            // Normalize axis
    ...
    static QuaternionRotation lerp( const QuaternionRotation& qa, const QuaternionRotation& qb, T t )
    {
        ...
        // Normalize
        T rLength = T( 1.0 ) /
                    ( std::sqrt( q._q[ 0 ] * q._q[ 0 ] + q._q[ 1 ] * q._q[ 1 ] + q._q[ 2 ] * q._q[ 2 ] + q._q[ 3 ] *
                                 q._q[ 3 ] ) );
    ...
    static QuaternionRotation slerp( const QuaternionRotation& q1, const QuaternionRotation& q2, T t )
    {
        ...
        T cos_q1 = q1._q[ 0 ] * q2._q[ 0 ] + q1._q[ 1 ] * q2._q[ 1 ] + q1._q[ 2 ] * q2._q[ 2 ] + q1._q[ 3 ] *
                   q2._q[ 3 ];
